...
const {
  isDateFormatValid,
  isNumeric,
} = require("../utilities/utilFunctions.js");
...
async function getAll(query) {
  if (query.selectValue !== undefined && query.textValue !== undefined) {
    var querryString = "WHERE ";
    if (
      query.selectValue == "protivnik" ||
      query.selectValue == "rezultat" ||
      query.selectValue == "datum_borbe"
    ) {
      querryString += "borbe.";
    } else {
      querryString += "borci.";
    ...
    if (
      query.selectValue == "datum_rođenja" ||
      query.selectValue == "datum_borbe" ||
      query.selectValue == "broj_značajnih_udaraca_po_minuti" ||
      query.selectValue == "broj_rušenja_po_minuti"
    )
      querryString += `${query.selectValue} = '${query.textValue}'`;
    else if (query.selectValue == "protivnik")
      querryString += `b.ime || ' ' || b.prezime LIKE '%${query.textValue}%'`;
    else querryString += `${query.selectValue} LIKE '%${query.textValue}%'`;

    if (query.selectValue == "sva") {
      let tmpStr = () => {
        if (isNumeric(query.textValue)) {
          return `OR borci.broj_značajnih_udaraca_po_minuti = ${query.textValue}\
                OR borci.broj_rušenja_po_minuti = ${query.textValue}\
                                                `;
        } else {
          if (isDateFormatValid(query.textValue)) {
            return `OR borci.datum_rođenja = '${query.textValue}'\
                    OR borbe.datum_borbe = '${query.textValue}'\
                `;
          }
        }
        return "";
      };
      querryString = `WHERE borci.ime LIKE '%${query.textValue}%'\
                      OR borci.prezime LIKE '%${query.textValue}%'\
                      OR borci.rekord LIKE '%${query.textValue}%'\
                      OR borci.preciznost_značajnih_udaraca LIKE '%${
                        query.textValue
                      }%'\
                      ${tmpStr()}\
                      OR borci.preciznost_rušenja LIKE '%${query.textValue}%'\
                      OR b.ime || ' ' || b.prezime LIKE '%${query.textValue}%'\
                      OR borbe.rezultat LIKE '%${query.textValue}%'\
                      `;
    }
  } else querryString = "";
...
function transformToJSON(inputDataArray) {
  ...
  function transformFightData(fight) {
      ...
      if (fight.protivnik_ime !== null && fight.protivnik_prezime !== null)
        tmp[0].prethodne_borbe.push({
          ...
          "@context": {
            ...
            protivnik: "competitor",
            datum_borbe: "startDate",
            rezultat: "result"
          },
          isAccessibleForFree: false,
          inLanguage: "en",
          sport: "MMA",
          description: "UFC Fight",
          sameAs: "https://www.ufc.com/events",
    ...
    const transformedFight = {
      "@type": "Person",
      "sameAs": "https://www.ufc.com/athlete/"+fight.ime+"-"+fight.prezime,
      id: fight.id,
      ime: fight.ime,
